...
# atm_mass in atomic mass unit (Dalton)
#print atm_mass
#atm_mass *= AMUTOAU
...
# read ground state Hessian
hessian = get_hessian_from_fchk("onp.fchk", N)
hessian_mwc = hessian / np.sqrt(M_mat) * me
Lambda, L = np.linalg.eig(hessian_mwc)
idx = Lambda.argsort(axis=0)
Lambda = Lambda[idx]
L = L.T[idx].T
print idx
...
for i in range(3*N):
    if Lambda[i] < 0.0:
        l[i] = -np.sqrt(-Lambda[i])
    else:
        l[i] = np.sqrt(Lambda[i])
l = l.reshape(-1,1)
l = l * FREQAU * 0.01 / C0 / 2.0 / PI

# test lambda dimensions
#Lambda = 1.0 / (100. * 2. * PI * C0 * l / FREQAU) ** 2.0
Lambda = Lambda * (2.0 * PI)**2
...
Vx = ex_grad - gs_grad
Vx = Vx.reshape(-1,1)
DQ = (1.0 / Lambda) * np.dot(np.transpose(L), Vx / np.sqrt(M_vec))
me = 0.000548579903 # Dalton
D = (Lambda/me)**.25 * DQ
print D
raise SystemExit
